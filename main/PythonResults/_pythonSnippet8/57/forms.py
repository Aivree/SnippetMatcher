...
class ProductForm(forms.ModelForm):
    ...
    def set_initial(self, product_class, parent, kwargs):
        ...
        if parent:
            ...
            # if its a product has a parent 
            self.set_initial_values_for_child(product_class,parent,kwargs)
    ...
    def set_initial_attribute_values(self, product_class, kwargs):
        ...
        for attribute in product_class.attributes.all():
            try:
                ...
                if attribute.is_sku:
                    # if it's a sku attributes
                    kwargs['initial']['attr_sku_%s' % attribute.code] = value
                else:
    ...
    def add_attribute_fields(self, product_class, is_parent=False):
        ...
        for attribute in product_class.attributes.all():
            ...
            if field:
                if attribute.is_sku:
                    self.fields['attr_sku_%s' % attribute.code] = field
                else:
                ...
                if is_parent:
                    try:
                        ...
                        self.fields['attr_sku_%s' % attribute.code].required = False
    def set_initial_values_for_child(self,product_class,parent,kwargs):
        """
        set the non-sku attribute values for child product from its parent relative 
        attribute values
        """
        instance = kwargs.get('instance')
        if instance is None:
            ...
            child = instance
        kwargs['initial']['title'] = child.title or parent.title
        kwargs['initial']['upc'] = child.upc or parent.upc
        # we set the value for child no matter there is a instance
        for attribute in product_class.attributes.all():
            try:
                parent_value = parent.attribute_values.get(
                    attribute=attribute).value
            except exceptions.ObjectDoesNotExist:
                parent_value = None
            try:
                child_value = child.attribute_values.get(
                    attribute=attribute).value
            except exceptions.ObjectDoesNotExist:
                child_value = None
            # prefer the value set in child product
            value = child_value or parent_value
            if attribute.is_sku:
                # if it's a sku attributes
                pass
            else:
    ...
    def _post_clean(self):
        ...
        for attribute in product_class.attributes.all():
            if attribute.is_sku:
                # if is sku attribute
                field_name = 'attr_sku_%s' % attribute.code
            else:
